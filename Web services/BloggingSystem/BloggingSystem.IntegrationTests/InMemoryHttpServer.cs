...
namespace BloggingSystem.IntegrationTests
{
    internal class InMemoryHttpServer
    {
        ...
        public HttpResponseMessage Put(string requestUrl, string data, IDictionary<string, string> headers = null, string mediaType = "application/json")
        {
            var url = requestUrl;
            var request = new HttpRequestMessage();
            request.RequestUri = new Uri(this.baseUrl + url);
            ...
            request.Content = new StringContent(data == null ? string.Empty : JsonConvert.SerializeObject(data));
            request.Content.Headers.ContentType = new MediaTypeHeaderValue(mediaType);
            if (headers != null)
            {
                foreach (var header in headers)
